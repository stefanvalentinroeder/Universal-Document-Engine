...
def enum_values(enum_class: type[Enum]) -> list[str]:
    """Return the explicit persisted values for a domain enum.

    SQLAlchemy otherwise persists enum member names by default. The PostgreSQL
    types created by the migration use the explicit lowercase ``.value``
    strings, so every ORM mapping must use this callable as well.
    """
...
class Tenant(TimestampedRecord, Base):
    ...
    state: Mapped[TenantState] = mapped_column(
        SqlEnum(TenantState, name="tenant_state", values_callable=enum_values),
        nullable=False,
...
class TenantMembership(TimestampedRecord, Base):
    ...
    role: Mapped[MembershipRole] = mapped_column(
        SqlEnum(MembershipRole, name="membership_role", values_callable=enum_values),
        nullable=False,
...
class Document(TimestampedRecord, Base):
    ...
    state: Mapped[DocumentState] = mapped_column(
        SqlEnum(DocumentState, name="document_state", values_callable=enum_values),
        nullable=False,
...
class RuleSet(TimestampedRecord, Base):
    ...
    state: Mapped[RuleSetState] = mapped_column(
        SqlEnum(RuleSetState, name="rule_set_state", values_callable=enum_values),
        nullable=False,
...
class ProcessingRun(TimestampedRecord, Base):
    ...
    execution_mode: Mapped[ProcessingExecutionMode] = mapped_column(
        SqlEnum(
            ...
            name="processing_execution_mode",
...
class ReviewTask(TimestampedRecord, Base):
    ...
    state: Mapped[ReviewTaskState] = mapped_column(
        SqlEnum(ReviewTaskState, name="review_task_state", values_callable=enum_values),
        nullable=False,
